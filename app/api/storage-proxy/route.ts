...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Firebase Storage GET error: ${response.status} ${response.statusText}`,
      )
      return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Firebase Storage POST error: ${response.status} ${response.statusText}`,
      )
  ...
  return new NextResponse(null, {
    headers: {
      ...
      'Access-Control-Allow-Headers':
        'Content-Type, Authorization, X-Goog-Upload-Protocol, X-Goog-Upload-Content-Type',
    },
  })
}
