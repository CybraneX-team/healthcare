...
import { PatientData, System, Test, findTestInSystem, findTestInSystems } from '../types/medical-data';
...
export const useMedicalData = (data?: PatientData) => {
  ...
  const systems = useMemo(() => {
    ...
    return {
      ...
      neurological: data.systems.find(s => s.name === "Neurological / Autonomic"),
      ...
      neurotransmitters: data.systems.find(s => s.name === "Neurotransmitters"),
  ...
  const getHealthStatus = (value: number, referenceRange?: { low: number; high: number }) => {
    if (!referenceRange) return { status: "Unknown", color: "bg-gray-100 text-gray-800" };
  ...
  return {
    ...
    testCount: data?.systems.reduce((count, system) => count + system.tests.length, 0) || 0
  };
};
    ...
    return {
      vo2max: {
        ...
        status: cardiovascularData.vo2max?.reference_range ? 
          getHealthStatus(vo2max || 0, cardiovascularData.vo2max.reference_range) : undefined
      },
      heartRateVariability: {
        value: hrv || 45,
        unit: cardiovascularData.heartRateVariability?.unit || "ms",
        status: cardiovascularData.heartRateVariability?.reference_range ? 
          getHealthStatus(hrv || 0, cardiovascularData.heartRateVariability.reference_range) : undefined
      },
      troponin: {
        value: troponin || 0.02,
        unit: cardiovascularData.troponin?.unit || "ng/mL",
        status: cardiovascularData.troponin?.reference_range ? 
          getHealthStatus(troponin || 0, cardiovascularData.troponin.reference_range) : undefined
      }
    };
    ...
    return {
      ...
      creatinine: {
        ...
        status: renalData.creatinine?.reference_range ? 
          getHealthStatus(creatinine || 0, renalData.creatinine.reference_range) : undefined
      },
      bun: {
        value: bun || 29,
        unit: renalData.bun?.unit || "mg/dL",
        status: renalData.bun?.reference_range ? 
          getHealthStatus(bun || 0, renalData.bun.reference_range) : undefined
      },
      bunCreatinineRatio: bun && creatinine ? Math.round((bun / creatinine) * 10) / 10 : 24.2
    };
