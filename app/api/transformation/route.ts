...
export async function POST(req: Request) {
    try {
        ...
        const currentWeight = formData.get("currentWeight") as string;        if (!file || !targetWeight || !currentWeight) {
            return NextResponse.json({ error: "Image, currentWeight, and targetWeight are required" }, { status: 400 });
        }
        ...
        if (isNaN(currentWeightNum) || isNaN(targetWeightNum) || currentWeightNum <= 0 || targetWeightNum <= 0) {
            return NextResponse.json({ error: "Please provide valid weight values" }, { status: 400 });
        }

        if (currentWeightNum > 300 || targetWeightNum > 300) {
            return NextResponse.json({ error: "Weight values seem unusually high. Please check your input." }, { status: 400 });
        }
        ...
        if (!analysisText) {
            return NextResponse.json({ 
                error: "No analysis generated by AI. Please try again with a different image." 
            }, { status: 500 });
        }
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                analysisData = {
                    ...
                    recommendations: ["Consult with healthcare professionals", "Follow a balanced approach"],
            ...
            analysisData = {
                ...
                key_changes: isWeightLoss ? 
                    ["Reduced body fat percentage", "Improved muscle definition", "Better posture"] : 
